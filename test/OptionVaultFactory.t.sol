...
contract OPairFactoryTest is Test {
    ...
    function test_createPair_storesInPairsMapping() public {
        ...
        bytes32 key = keccak256(
            abi.encode(address(weth), address(usdc), STRIKE, EXPIRY, true)
        );
    ...
    function test_createPair_callAndPutAreDifferentAddresses() public {
        vm.startPrank(admin);
        address callPair = factory.createPair(
            address(weth),
            address(usdc),
            STRIKE,
            EXPIRY,
        ...
        address putPair = factory.createPair(
            address(weth),
            address(usdc),
            STRIKE,
            EXPIRY,
    ...
    function test_createPair_revertsExpiryTooSoon() public {
